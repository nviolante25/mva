...
void getClicks(Window w1, Window w2,
    ...
    // ------------- TODO/A completer ----------
    Window window_clicked;
    IntPoint2 point;
    int sw;
    const int RIGHT_BUTTON = 3;
    int mouse_button = 1;

    while (mouse_button != RIGHT_BUTTON) {
        mouse_button = anyGetMouse(point, window_clicked, sw);
        if (window_clicked == w1) {
            pts1.push_back(point);
        }
        else if (window_clicked == w2) {
...
void panorama(const Image<Color,2>& I1, const Image<Color,2>& I2,
    ...
    for (int x=0; x < I.width(); x++) {
        for (int y=0; y< I.height(); y++) {
            Vector<float> vec(3);
            // vec must be in the same coordinates as I2
            vec[0] = x + x0;
            vec[1] = y + y0;
            vec[2] = 1;
            auto v_in_I1 = H_inv * vec;
            v_in_I1 /= v_in_I1[2];
            if (0 <= vec[0] && vec[0] < I2.width() && 0 <= vec[1] && vec[1] < I2.height()) {
                if (0 <= v_in_I1[0] && v_in_I1[0] <= I1.width() && 0 <= v_in_I1[1] && v_in_I1[1] < I1.height()) {
                    auto av1 = I1.interpolate(v_in_I1[0], v_in_I1[1]);
                    RGB<float> av2 = I2(vec[0], vec[1]);
                    I(x,y) = (av1 + av2) / 2;
                }
                else {
                    I(x, y) = I2(vec[0], vec[1]);
                }
            }
            else if (0 <= v_in_I1[0] && v_in_I1[0] < I1.width() && 0 <= v_in_I1[1] && v_in_I1[1] < I1.height()) {
                I(x, y) = I1.interpolate(v_in_I1[0], v_in_I1[1]);
            }
